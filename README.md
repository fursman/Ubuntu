...
| `macbook/install.sh` | MacBook Pro quirks: stops PipeWire suspending the CS8409 sound card (closing playback freezes the microphone), and makes libinput palm rejection work at all (the applespi keyboard reports no vendor id, so disable-while-typing never engages). Audio fix is per-user; the input fix needs sudo. | Yes -- inert on other hardware, see [macbook/README.md](macbook/README.md) |
