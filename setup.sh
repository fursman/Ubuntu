...
declare -A CONFIG_MAP=(
    ...
    ["configs/hypr/hyprland.lua"]="$HOME/.config/hypr/hyprland.lua"
...
# -- Lua forms of the three machine-local files ------------------------------
# Hyprland 0.55 deprecated hyprlang and 0.57 removes it. hyprland.lua is loaded
# in preference to hyprland.conf when both exist, so both sets are generated
# during the transition: whichever config Hyprland picks finds its local files.
# Derived from the .conf files just written, so the two cannot drift.
python3 - "$HOME/.config/hypr" <<'LUAGEN'
import re, sys, os
d = sys.argv[1]
...
with open(os.path.join(d, "env-local.lua"), "w") as f:
    f.write("-- Generated by setup.sh from env-local.conf. Do not edit.\n")
    for l in lines("env-local.conf"):
        m = re.match(r"\s*env\s*=\s*([^,]+),(.*)$", l)
        if m:
            f.write('hl.env("%s", "%s")\n' % (m.group(1).strip(), m.group(2).strip()))

cmds = [m.group(1).strip() for l in lines("polkit-local.conf")
        for m in [re.match(r"\s*exec-once\s*=\s*(.*)$", l)] if m]
with open(os.path.join(d, "polkit-local.lua"), "w") as f:
    f.write("-- Generated by setup.sh from polkit-local.conf. Do not edit.\n")
    if cmds:
        f.write('hl.on("hyprland.start", function()\n')
        for c in cmds:
            f.write('    hl.exec_cmd("%s")\n' % c)
        f.write("end)\n")
LUAGEN
info "-> ~/.config/hypr/{env-local,polkit-local}.lua"

# local.lua is the hand-edited one, so it is created once and never rewritten,
# exactly like local.conf.
HYPR_LOCAL_LUA="$HOME/.config/hypr/local.lua"
if [ -f "$HYPR_LOCAL_LUA" ]; then
    info "-> ~/.config/hypr/local.lua (kept, not overwritten)"
else
    cat > "$HYPR_LOCAL_LUA" <<'LOCALLUAEOF'
-- Machine-local Hyprland overrides, in Lua. Loaded last, so anything here
-- beats the shared config. setup.sh creates this file once and never
-- overwrites it.
--
-- Example -- needed when the display hangs off one of several GPUs, where
-- hardware cursor planes fail with "Backend requires blit, but cursor blit
...
LOCALLUAEOF
    info "-> ~/.config/hypr/local.lua (created)"
fi
